...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { transcript, mission, round, mode, debateState } = body;
    ...
    const debateStateText = `
Μνήμη συζήτησης (για να μην ξαναμετράς τα ίδια):
- Έχει ήδη βαθμολογηθεί Εικόνα σε προηγούμενο γύρο; ${debateState?.imageScored ? "ΝΑΙ" : "ΟΧΙ"}
- Έχει ήδη βαθμολογηθεί Μεταφορά σε προηγούμενο γύρο; ${debateState?.metaphorScored ? "ΝΑΙ" : "ΟΧΙ"}
`.trim();
...
{"criteria":{"Θέση":0-4,"Επιχειρηματολογία":0-6,"Εικόνα":0-2,"Μεταφορά":0-2,"Παράδειγμα":0-3,"Αντίρρηση":0-4},"feedback":"κείμενο"}
Χωρίς markdown ή σχόλια.
Συμπλήρωσε βαθμούς ΜΟΝΟ για τα κριτήρια που εμφανίζονται στη λίστα "Κριτήρια γύρου" (τα υπόλοιπα βάλε τα 0).
...
- Βαθμολόγησε "Εικόνα" από 0 έως 2 και "Μεταφορά" από 0 έως 2, ξεχωριστά.
- Δώσε 0 αν δεν χρησιμοποιήθηκε ή είναι διακοσμητική/άσχετη.
- Δώσε 1 αν υπάρχει προσπάθεια αλλά η σύνδεση με τη θέση/επιχείρημα είναι αδύναμη.
- Δώσε 2 αν η χρήση είναι καθαρά συνδεδεμένη και ενισχύει τη θέση/επιχείρημα.
- Μην δώσεις κανένα μπόνους.
...
// ✅ Καθαρά, νέα ονόματα — χωρίς παλιά mapping
const C = {
  Θέση: Number(c["Θέση"]) || 0,
  Επιχειρηματολογία: Number(c["Επιχειρηματολογία"]) || 0,
  Εικόνα: Number(c["Εικόνα"]) || 0,
  Μεταφορά: Number(c["Μεταφορά"]) || 0,
  Παράδειγμα: Number(c["Παράδειγμα"]) || 0,
  Αντίρρηση: Number(c["Αντίρρηση"]) || 0
};

// ✅ Μηδενισμός κριτηρίων που δεν ισχύουν στον τρέχοντα γύρο
const activeKeys = new Set(activeCriteria.map(x => x.key));
for (const key of Object.keys(C)) {
  if (!activeKeys.has(key)) C[key] = 0;
} 
